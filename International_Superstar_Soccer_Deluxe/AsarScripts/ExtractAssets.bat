...
set asarVer=asar
set ROMName=
set ROMName0=ISSD_USA.sfc
set ROMName1=ISSD_PAL.sfc
set ROMName2=ISSD_JAPAN.sfc
set MemMap=lorom
...
:Start
echo Place a headerless ISSD ROM with one of the following names in this folder, then type the number representing what ROM to extract from.
echo 0 = "%ROMName0%" (ISSD USA)
echo 1 = "%ROMName1%" (ISSD PAL)
echo 2 = "%ROMName2%" (ISSD Japan)
...
if %PointerSet% equ 42 goto :MoveCOAMD
if %PointerSet% equ 48 goto :MoveGarbageData
...
echo:^^!LoopCounter #= 0 >> TEMP1.asm
echo:dd snestopc(DataPointers) : dw ^^!FilesToCombine : dw ^^!FileType : db "ISSD    " : db "RDC V1.0.0" >> TEMP1.asm
